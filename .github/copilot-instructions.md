...
**Core KV Interface** (`kv.go`): Defines CRUD operations (`Get`, `Put`, `Insert`, `Remove`, `Query`, `Batch`) and uses `lexkey.LexKey` for hierarchical partitioning. All operations are context-aware for cancellation/timeouts. **The interface is the lowest common denominator of all backends:** do not add methods or documented guarantees that cannot be implemented by every backend (Pebble, Redis, Azure).

**Pluggable Backends** (`pkg/storage/`):

- **Pebble** (`pkg/storage/pebble/store.go`): Embedded RocksDB-like store; default for local development
- **Redis** (`pkg/storage/redis/store.go`): In-memory with TTL support
- **Azure Tables** (`pkg/storage/azure/store.go`): Cloud-native NoSQL
...
**Docker Setup** (see root `compose.yml` for fazure and Redis):
